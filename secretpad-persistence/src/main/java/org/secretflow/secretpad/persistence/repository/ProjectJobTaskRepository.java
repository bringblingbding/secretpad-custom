...
public interface ProjectJobTaskRepository extends BaseRepository<ProjectTaskDO, ProjectTaskDO.UPK> {
     ...
     */
    @Query(value = "select t.* from project_job_task t join project_job j on t.job_id=j.job_id where t.project_id=:projectId and t.graph_node_id=:graphNodeId order by t.id desc limit 1", nativeQuery = true)
